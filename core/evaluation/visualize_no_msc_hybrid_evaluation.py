...
DEFAULT_METRICS = ["precision@10", "recall@10", "mrr@10", "ndcg@10"]
COLORS = {
    "hybrid_50_50": "#2563eb",
    "hybrid_70_30": "#dc2626",
}
...
def metric_columns(rows):
    return [
        ...
        if key.startswith(("precision@", "recall@", "mrr@", "ndcg@"))
    ] or DEFAULT_METRICS


def svg_relevant_hits(rows_by_scenario, metric):
    width = 920
    height = 320
    margin_left = 58
    margin_top = 28
    ...
    scenario_names = list(rows_by_scenario)
    counters = {
        scenario: Counter(int(float(row[metric])) for row in rows)
        for scenario, rows in rows_by_scenario.items()
    }
    k = int(metric.split("@", 1)[1]) if "@" in metric else 10
    labels = list(range(0, k + 1))
    max_count = max(max(counter.values()) for counter in counters.values()) or 1
    group_width = chart_width / len(labels)
    bar_width = min(30, group_width / (len(scenario_names) + 1.3))

    parts = [
        f'<svg viewBox="0 0 {width} {height}" role="img" aria-label="Relevant hits at {k} distribution">',
        '<rect width="100%" height="100%" fill="#ffffff"/>',
        f'<line x1="{margin_left}" y1="{margin_top + chart_height}" '
        f'x2="{margin_left + chart_width}" y2="{margin_top + chart_height}" stroke="#cbd5e1"/>',
    ]
    ...
    parts.append(f'<text x="{margin_left}" y="{height - 18}" font-size="13" fill="#475569">X axis: number of relevant hits in top {k}.</text>')
    parts.append("</svg>")
    return "\n".join(parts)
...
def write_report(
    ...
    metrics = metric_columns(summary_rows)
    precision_metric = next((metric for metric in metrics if metric.startswith("precision@")), "precision@10")
    mrr_metric = next((metric for metric in metrics if metric.startswith("mrr@")), "mrr@10")
    ndcg_metric = next((metric for metric in metrics if metric.startswith("ndcg@")), "ndcg@10")
    k_suffix = precision_metric.split("@", 1)[1] if "@" in precision_metric else "10"
    relevant_hits_metric = f"relevant_found@{k_suffix}"

    charts = [
        ("Summary Metrics", svg_bar_chart(summary_rows, metrics)),
        (f"Relevant Hits @{k_suffix}", svg_relevant_hits(rows_by_scenario, relevant_hits_metric)),
        (f"Precision @{k_suffix} Distribution", svg_histogram(rows_by_scenario, precision_metric)),
        (f"MRR @{k_suffix} Distribution", svg_histogram(rows_by_scenario, mrr_metric)),
        (f"nDCG @{k_suffix} Distribution", svg_histogram(rows_by_scenario, ndcg_metric)),
    ]
